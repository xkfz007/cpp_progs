...
int main(void)
{
    ...
    fx_bprint_init(&b, 0, 0);
    bprint_pascal(&b, 25);
    printf("Long text count only buffer: %u/%u\n", (unsigned)strlen(b.str), b.len);

    fx_bprint_init_for_buffer(&b, buf, sizeof(buf));
    bprint_pascal(&b, 25);
    printf("Long text count only buffer: %u/%u\n", (unsigned)strlen(buf), b.len);

    fx_bprint_init(&b, 0, -1);
    fx_bprint_strftime(&b, "%Y-%m-%d", &testtime);
    printf("strftime full: %u/%u \"%s\"\n", (unsigned)strlen(buf), b.len, b.str);
    fx_bprint_finalize(&b, NULL);

    fx_bprint_init(&b, 0, 8);
    fx_bprint_strftime(&b, "%Y-%m-%d", &testtime);
