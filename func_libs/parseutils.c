...
static void test_fx_small_strptime(void)
{
    int i;
    struct tm tm = { 0 };
    ...
    fx_log_set_level(FX_LOG_DEBUG);
    for (i = 0;  i < ARRAY_ELEMS(fmt_timespec_entries); i++) {
        char *p;
...
static void test_fx_parse_time(void)
{
    int i;
    int64_t tv;
    time_t tvi;
    ...
    for (i = 0;  i < ARRAY_ELEMS(time_string); i++) {
    ...
    for (i = 0;  i < ARRAY_ELEMS(duration_string); i++) {
...
int main(void)
{
    ...
    printf("\nTesting fx_small_strptime()\n");
    test_fx_small_strptime();

    printf("\nTesting fx_parse_time()\n");
    test_fx_parse_time();
